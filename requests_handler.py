    ...
    try:
        ...
        if file_path is None:
            ...
            if not file_path.exists():
                # Если файл не найден в текущей директории, ищем в родительской
                file_path = script_dir.parent / "settings.txt"
                if not file_path.exists():
                    raise FileNotFoundError("Файл settings.txt не найден")
...
async def send_request(cookies_str: str = None, headers=None, type_: RequestTypes = RequestTypes.GET,
    ...
    # инициализация асинхрон��ого клиента с настройками
    client_params = {
